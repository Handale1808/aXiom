...
export function generateSvgString(traits: IPhysicalTraits): string {
  ...
  const tails =
    traits.tails > 0
      ? Array.from({ length: traits.tails })
          .map((_, i) => {
        ...
        `;
            ...
            return `<path d="${tailPath}" fill="none" stroke="${tailColor}" stroke-width="${jitter(4, i * 100 + 30, 1)}" stroke-linecap="round" opacity="${jitter(0.7, i * 100 + 4, 0.1)}" transform="translate(${originX}, ${originY}) rotate(${angle})" />`;
          })
          .join("")
      : "";
  ...
  const wings =
    traits.wings > 0
      ? Array.from({ length: traits.wings })
          .map((_, i) => {
            ...
            return `<polygon points="0,-15 -8,0 8,0" fill="${wingColor}" opacity="${jitter(0.6, i * 400 + 3, 0.1)}" transform="translate(${curvePoint.x}, ${curvePoint.y - 20}) rotate(${(perpAngle * 180) / Math.PI})" />`;
          })
          .join("")
      : "";
    ...
    traits.eyes > 0
      ? Array.from({ length: traits.eyes })
          .map((_, i) => {
            ...
            return `<circle cx="${eyeX}" cy="${eyeY}" r="${jitter(3, i * 500 + 2)}" fill="white" />`;
          })
          .join("")
      : "";
