...
aXiom is a full-stack feedback analysis application built with a three-tier architecture. The concerns are separated into distinct layers.
...
| Field | Note |
|-------|------|
| `_id` | MongoDB default for uniqueness + timestamp |
| `email` | Optional; future: separate users table |
| `analysis` | Embedded for atomic retrieval |
| `tags` | Array for simplicity; future: tag metadata table |
| `priority` | Strings for readability; future: priority table with descriptions |
...
| `tags` | 2-5 single-word nouns |
| `urgencyIndicators` | Future: store these |
| `customerSatisfied` | Future: store these |
| `safetyThreat` | Future: store these |
